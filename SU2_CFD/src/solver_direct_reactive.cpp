...
CReactiveEulerSolver::CReactiveEulerSolver(): CSolver(), nSpecies(), nPrimVarLim(), space_centered(), implicit(), grid_movement(),
                                              least_squares(), second_order(), limiter(), turbulent(), tkeNeeded(),
                                              Density_Inf(), Pressure_Inf(), Temperature_Inf(), T_INDEX_PRIM(), VX_INDEX_PRIM(),
                                              P_INDEX_PRIM(), RHO_INDEX_PRIM(), H_INDEX_PRIM(), A_INDEX_PRIM(), RHOS_INDEX_PRIM(),
                                              RHO_INDEX_SOL(), RHOVX_INDEX_SOL(), RHOE_INDEX_SOL(), RHOS_INDEX_SOL(), T_INDEX_GRAD(),
                                              VX_INDEX_GRAD(), P_INDEX_GRAD(), T_INDEX_LIM(), VX_INDEX_LIM(), P_INDEX_LIM() {
...
CReactiveEulerSolver::CReactiveEulerSolver(CGeometry* geometry, CConfig* config, unsigned short iMesh): CSolver() {
  ...
  std::fill(Point_Max, Point_Max + nVar, 0);
...
void CReactiveEulerSolver::Check_FreeStream_Solution(CConfig* config) {
  ...
  for(unsigned long iPoint = 0; iPoint < nPoint; ++iPoint) {
    ...
    if(nonPhys) {
      ...
      rho = library->ComputeDensity(Temperature_Inf, Pressure_Inf, MassFrac_Inf);
...
void CReactiveEulerSolver::SetInitialCondition(CGeometry** geometry, CSolver*** solver_container, CConfig *config, unsigned long ExtIter) {
    ...
    for(iMesh = 1; iMesh <= config->GetnMGLevels(); ++iMesh) {
      for (iPoint = 0; iPoint < geometry[iMesh]->GetnPoint(); ++iPoint) {
        ...
        for(iChildren = 0; iChildren < geometry[iMesh]->node[iPoint]->GetnChildren_CV(); ++iChildren) {
          ...
          auto Solution_Fine = solver_container[iMesh-1][REACTIVE_SOL]->node[Point_Fine]->GetSolution();
          for(iVar = 0; iVar < nVar; ++iVar) {
            Solution[iVar] = Solution_Fine[iVar]*Area_Children/Area_Parent;
          }
        }
        solver_container[iMesh][REACTIVE_SOL]->node[iPoint]->SetSolution(Solution);
      }
      solver_container[iMesh][REACTIVE_SOL]->Set_MPI_Solution(geometry[iMesh], config);
    }

    /*--- Interpolate the turblence variable also, if needed ---*/
    if(rans) {
      unsigned short nVar_Turb = solver_container[MESH_0][TURB_SOL]->GetnVar();
      su2double Solution_Turb[nVar_Turb];
      for(iMesh = 1; iMesh <= config->GetnMGLevels(); ++iMesh) {
        for(iPoint = 0; iPoint < geometry[iMesh]->GetnPoint(); ++iPoint) {
          Area_Parent = geometry[iMesh]->node[iPoint]->GetVolume();
          for(iVar = 0; iVar < nVar_Turb; ++iVar)
            Solution_Turb[iVar] = 0.0;
          for(iChildren = 0; iChildren < geometry[iMesh]->node[iPoint]->GetnChildren_CV(); ++iChildren) {
            Point_Fine = geometry[iMesh]->node[iPoint]->GetChildren_CV(iChildren);
            Area_Children = geometry[iMesh-1]->node[Point_Fine]->GetVolume();
            Solution_Fine = solver_container[iMesh-1][TURB_SOL]->node[Point_Fine]->GetSolution();
            for(iVar = 0; iVar < nVar_Turb; ++iVar) {
              Solution_Turb[iVar] += Solution_Fine[iVar]*Area_Children/Area_Parent;
            }
          }
          solver_container[iMesh][TURB_SOL]->node[iPoint]->SetSolution(Solution_Turb);
        }
  ...
  if(dual_time && (ExtIter == 0 || (restart && static_cast<long>(ExtIter) == config->GetUnst_RestartIter()))) {
    ...
    for(iMesh = 0; iMesh <= config->GetnMGLevels(); ++iMesh) {
      for(iPoint = 0; iPoint < geometry[iMesh]->GetnPoint(); ++iPoint) {
        solver_container[iMesh][REACTIVE_SOL]->node[iPoint]->Set_Solution_time_n();
        solver_container[iMesh][REACTIVE_SOL]->node[iPoint]->Set_Solution_time_n1();
        if (rans) {
    ...
    if(restart && static_cast<long>(ExtIter) == config->GetUnst_RestartIter() && config->GetUnsteady_Simulation() == DT_STEPPING_2ND) {
      ...
      solver_container[MESH_0][REACTIVE_SOL]->LoadRestart(geometry, solver_container, config, SU2_TYPE::Int(config->GetUnst_RestartIter() - 1));
...
void CReactiveEulerSolver::SetResidual_DualTime(CGeometry* geometry, CSolver** solver_container, CConfig* config,
  ...
  if(!grid_movement) {
    ...
    for(iPoint = 0; iPoint < nPointDomain; ++iPoint) {
      ...
      LinSysRes.AddBlock(iPoint, Residual);
      if(implicit) {
        throw Common::NotImplemented("Implicit computation for dual time strategies not implemented");
...
void CReactiveEulerSolver::Source_Residual(CGeometry* geometry, CSolver** solver_container, CNumerics* numerics, CNumerics* second_numerics,
  ...
  for(iPoint = 0; iPoint < nPointDomain; ++iPoint) {
    ...
    if(turbulent)
      numerics->SetTurbVar(solver_container[TURB_SOL]->node[iPoint]->GetSolution(), solver_container[TURB_SOL]->node[iPoint]->GetSolution());
...
void CReactiveEulerSolver::BC_Euler_Wall(CGeometry* geometry, CSolver** solver_container, CNumerics* numerics,
  ...
  su2double Area, Pressure, turb_ke;
  ...
  /*--- Loop over all the vertices on this boundary (val_marker) ---*/
	for(iVertex = 0; iVertex < geometry->nVertex[val_marker]; ++iVertex) {
      ...
      if(tkeNeeded) {
        for(iDim = 0; iDim < nDim; ++iDim)
          Residual[RHOVX_INDEX_SOL + iDim] += (2.0/3.0)*node[iPoint]->GetDensity()*turb_ke*UnitNormal[iDim]*Area;
      }
      ...
      if(implicit) {
        ...
        su2double a1 = node[iPoint]->GetEnergy() + turb_ke;
...
void CReactiveEulerSolver::BC_Supersonic_Inlet(CGeometry* geometry, CSolver** solver_container, CNumerics* conv_numerics,
  ...
  if(tkeNeeded)
    V_inlet[H_INDEX_PRIM] += GetTke_Inf();
  ...
  /*--- Loop over all the vertices on this boundary (val_marker) ---*/
	for(iVertex = 0; iVertex < geometry->nVertex[val_marker]; ++iVertex) {
      ...
      /*--- Viscous contribution ---*/
        ...
        if(turbulent)
          visc_numerics->SetTurbKineticEnergy(solver_container[TURB_SOL]->node[iPoint]->GetSolution(CReactiveTurbVariable::TURB_KINE_INDEX),
                                              solver_container[TURB_SOL]->node[iPoint]->GetSolution(CReactiveTurbVariable::TURB_KINE_INDEX));
...
//
/*!
  *\brief Supersonic Outlet boundary conditions.
  */
//
//
  ...
  for(iVertex = 0; iVertex < geometry->nVertex[val_marker]; ++iVertex) {
    ...
    if(geometry->node[iPoint]->GetDomain()) {
      ...
      if(Mach_Exit >= 1.0) {
        ...
        if(tkeNeeded)
          V_outlet[H_INDEX_PRIM] += GetTke_Inf();
...
void CReactiveNSSolver::Preprocessing(CGeometry* geometry, CSolver** solver_container, CConfig* config, unsigned short iMesh,
  ...
  su2double StrainMag_Max = 0.0, Omega_Max = 0.0;
  for(iPoint = 0; iPoint < nPoint; ++iPoint) {
    solver_container[REACTIVE_SOL]->node[iPoint]->SetVorticity(limiter_visc);
    solver_container[REACTIVE_SOL]->node[iPoint]->SetStrainMag(limiter_visc);

    StrainMag = solver_container[REACTIVE_SOL]->node[iPoint]->GetStrainMag();
    Vorticity = solver_container[REACTIVE_SOL]->node[iPoint]->GetVorticity();
    Omega = std::sqrt(Vorticity[0]*Vorticity[0] + Vorticity[1]*Vorticity[1] + Vorticity[2]*Vorticity[2]);

    StrainMag_Max = std::max(StrainMag_Max, StrainMag);
  ...
  if(config->GetConsole_Output_Verb() == VERB_HIGH) {
    #ifdef HAVE_MPI
      SU2_MPI::Allreduce(MPI_IN_PLACE, &ErrorCounter, 1, MPI_UNSIGNED_LONG, MPI_SUM, MPI_COMM_WORLD);
      SU2_MPI::Allreduce(MPI_IN_PLACE, &StrainMag_Max, 1, MPI_DOUBLE, MPI_MAX, MPI_COMM_WORLD);
      SU2_MPI::Allreduce(MPI_IN_PLACE, &Omega_Max, 1, MPI_DOUBLE, MPI_MAX, MPI_COMM_WORLD);
    #endif
    if(iMesh == MESH_0) {
      config->SetNonphysical_Points(ErrorCounter);
      solver_container[REACTIVE_SOL]->SetStrainMag_Max(StrainMag_Max);
      solver_container[REACTIVE_SOL]->SetOmega_Max(Omega_Max);
    }
  }
...
unsigned long CReactiveNSSolver::SetPrimitive_Variables(CSolver** solver_container, CConfig* config, bool Output) {
 ...
 for(iPoint = 0; iPoint < nPoint; ++iPoint) {
   /*--- Retrieve the value of eddy viscosity and turbulent kinetic energy (if need it) ---*/
   if(turbulent) {
   ...
   /*--- Compressible flow, primitive variables nSpecies + nDim + 5, (T, vx, vy, vz, P, rho, h, a, Y1,....YNs) ---*/
   RightSol = node[iPoint]->SetPrimVar(eddy_visc, turb_ke, config);
...
void CReactiveNSSolver::SetTime_Step(CGeometry* geometry, CSolver** solver_container, CConfig* config,
  ...
  for(iEdge = 0; iEdge < geometry->GetnEdge(); ++iEdge) {
    ...
    Mean_Gamma = 0.5*((node[iPoint]->GetdPdU()[RHOE_INDEX_SOL] + 1.0) + (node[jPoint]->GetdPdU()[RHOE_INDEX_SOL] + 1.0));
  ...
  for(iMarker = 0; iMarker < geometry->GetnMarker(); ++iMarker) {
    if(config->GetMarker_All_KindBC(iMarker) != INTERNAL_BOUNDARY) {
      for(iVertex = 0; iVertex < geometry->GetnVertex(iMarker); ++iVertex) {
        ...
        /*--- Viscous contribution ---*/
      	Lambda_1 = 4.0/3.0*(Mean_LaminarVisc + Mean_EddyVisc);
        Mean_Gamma = node[iPoint]->GetdPdU()[RHOE_INDEX_SOL] + 1.0;
        Lambda_2 = (1.0 + (Prandtl_Lam/Prandtl_Turb)*(Mean_EddyVisc/Mean_LaminarVisc))*(Mean_Gamma*Mean_LaminarVisc/Prandtl_Lam);
      	Lambda = (Lambda_1 + Lambda_2)*Area*Area/Mean_Density;
      	if(geometry->node[iPoint]->GetDomain())
          node[iPoint]->AddMax_Lambda_Visc(Lambda);
...
void CReactiveNSSolver::SetNondimensionalization(CGeometry* geometry, CConfig* config, unsigned short iMesh) {
  ...
  if(tkeNeeded) {
    su2double Tke_FreeStream = 3.0/2.0*(config->GetModVel_FreeStream()*config->GetModVel_FreeStream()*
...
void CReactiveNSSolver::Viscous_Residual(CGeometry* geometry, CSolver** solution_container, CNumerics* numerics,
  ...
  for(iEdge = 0; iEdge < geometry->GetnEdge(); ++iEdge) {
    ...
    /*--- Set turbulent kinetic energy ---*/
    if(turbulent)
      numerics->SetTurbKineticEnergy(solver_container[TURB_SOL]->node[iPoint]->GetSolution(CReactiveTurbVariable::TURB_KINE_INDEX),
                                     solver_container[TURB_SOL]->node[jPoint]->GetSolution(CReactiveTurbVariable::TURB_KINE_INDEX));
	...
	for(iVertex = 0; iVertex < geometry->nVertex[val_marker]; ++iVertex) {
      ...
      if(grid_movement) {
        ...
        for(iDim = 0; iDim < nDim; iDim++) {
          ...
          delta[iDim][iDim] = 1.0;
          for(jDim = 0; jDim < nDim; jDim++)
            tau[iDim][jDim] = laminar_viscosity*(Grad_Vel[iDim][jDim] + Grad_Vel[jDim][iDim] - TWO3*div_vel*delta[iDim][jDim]);
        }
