  ...
  if(implicit) {
    ...
    if(mF == mRef) {
      for(iVar = 0; iVar < nVar; ++iVar) {
        ...
        Scaling_Right_Der[iVar] = MachPol_Right_Der[iVar]*mR*(1.0 - mF)/mF;
      }
    }
    ...
    for(iVar = 0; iVar < nVar; ++iVar) {
      for(jVar = 0; jVar < nVar; ++jVar)
        val_Jacobian_i[iVar][jVar] += MeanSoundSpeed*((MassPlus_Left_Der[jVar] + MassPlus_Right_Der[jVar])*Density_i*Phi_i[iVar] +
                                                      (MassMinus_Left_Der[jVar] + MassMinus_Right_Der[jVar])*Density_j*Phi_j[iVar]);
    }

    /*--- Add pressure contribution to energy term ---*/
    for(iVar = 0; iVar < nVar; ++iVar)
      val_Jacobian_i[RHOE_INDEX_SOL][iVar] += MeanSoundSpeed*(0.5*(m12 + std::abs(m12))*S_i[iVar] + 0.5*(m12 - std::abs(m12))*S_j[iVar]);

    /*--- Add contribution to the diagonal term ---*/
    for(iVar = 0; iVar < nVar; ++iVar)
      val_Jacobian_i[iVar][iVar] += MeanSoundSpeed*m12;
    ...
    for(iDim = 0; iDim < nDim; ++iDim) {
      for(jVar = 0; jVar < nVar; ++jVar)
        val_Jacobian_i[RHOVX_INDEX_SOL + iDim][jVar] += UnitNormal[iDim]*(Pressure_Left_Der[jVar] + Pressure_Right_Der[jVar]);
    }

    /*--- Set Jacobian on point j ---*/
    for(iVar = 0; iVar < nVar; ++iVar)
      for(jVar = 0; jVar < nVar; ++jVar)
...
void CAvgGradReactive_Flow::ComputeResidual(su2double* val_residual, su2double** val_Jacobian_i,
  ...
  su2double Mean_Turb_ke; /*!< \brief Mean value of turbulent kinetic energy. */
...
void CSourceReactive::ComputeChemistry(su2double* val_residual, su2double** val_Jacobian_i, CConfig* config) {
  ...
  //su2double tau_mix = 1.0/(C_mu*TurbVar_i[CReactiveTurbVariable::TURB_OMEGA_INDEX]);
  //omega = library->GetMassProductionTerm(dim_temp, dim_rho, Ys, tau_mix);
