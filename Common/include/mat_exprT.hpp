...
/**
 * Definition of a wrapper base expression class MatExprT which derives from
...
public:
  ...
  /// Constructor
   MatExprT(DATA* data) : ExprT<DATA, ARG>(data) {}

  /// Destructor
   ~MatExprT(){}

  /// Number of columns
   std::size_t nbCols() const {return this->getData()->nbCols();}

  /// Number of rows
   std::size_t nbRows() const {return this->getData()->nbRows();}

  /// Accessor to individual entry
   typename ARG::TYPE at(std::size_t i, std::size_t j) const {return this->getData()->at(i,j);}
...
public:						\
   __OpName__ (MEETYPE(V1) v1, MEETYPE(V2) v2) :	    \
    MatExprT<__OpName__<V1,V2>, ETPLMAT(V1,V2)>(this), e1(v1), e2(v2) {}	\
    									\
     ETPLTYPE(V1) at(std::size_t i, std::size_t j) const {return e1.at(i,j) __op__ e2.at(i,j);} \
    									\
     std::size_t size()   const {return e1.size();}		\
     std::size_t nbCols() const {return e1.nbCols();}        \
     std::size_t nbRows() const {return e1.nbRows();}        \
									\
    MEETYPE(V1) e1;							\
    MEETYPE(V2) e2;							\
};
...
public:						\
   __OpName__ (__a1__ v1, __a2__ v2) :	    \
    MatExprT<__OpName__<V>, ETPL(V)>(this), e1(v1), e2(v2) {}	\
    									\
     ETPLTYPE(V) at(std::size_t i, std::size_t j) const {return __e1__ __op__ __e2__;} \
    									\
     std::size_t size()   const {return __e__ .size();}		\
     std::size_t nbCols() const {return __e__ .nbCols();}		\
     std::size_t nbRows() const {return __e__ .nbRows();}		\
									\
    __a1__ e1;								\
    __a2__ e2;								\
};
...
template __t1__							\
  inline __OpName__ __t2__ operator __op__ (__a1__ v1, __a2__ v2) \
{									\
...
class __OpName__ : public MatExprT< __OpName__<V>, ETPL(V)> { \
public:						\
   __OpName__ (MEETYPE(V) v1) :	    \
    MatExprT<__OpName__<V>, ETPL(V)>(this), e1(v1) {}	\
    									\
     ETPLTYPE(V) at(std::size_t i, std::size_t j) const {return __op__(e1.at(i,j));} \
    									\
     std::size_t size() const {return e1.size();}			\
     std::size_t nbCols() const {return e1.nbCols();}        \
     std::size_t nbRows() const {return e1.nbRows();}		\
...
template <typename V>						\
 inline __OpName__<V> __op__ (MEETYPE(V) v1)		\
{									\
...
public:
  ...
  /// Constructor
   MMulT (MEETYPE(V1) v1, MEETYPE(V2) v2) :
    MatExprT< MMulT<V1,V2>, ETPLMATMULT(V1,V2)>(this), ex1(v1), ex2(v2) {}

  /// Accessor to individual entry
   ETPLTYPE(V2) at(std::size_t i, std::size_t j) const
  {
    ETPLTYPE(V2) res = ETPLTYPE(V2)();
    const std::size_t nbCols = ex1.nbCols();
  ...
  /// Size of the array
   std::size_t size() const {return ex1.nbRows()*ex2.nbCols();}

  /// Number of columns
   std::size_t nbCols() const {return ex2.nbCols();}

  /// Number of rows
   std::size_t nbRows() const {return ex1.nbRows();}
...
template <typename V1, typename V2>
 inline MMulT<V1,V2> operator* (MEETYPE(V1) v1, MEETYPE(V2) v2)
{
...
public:
  ...
  /// Constructor
   VMMulT (EETYPE(V1) v1, MEETYPE(V2) v2) :
    MatExprT< VMMulT<V1,V2>, ETPLMATMULT(V1,V2)>(this), ex1(v1), ex2(v2) {}

  /// Accessor to individual entry
   ETPLTYPE(V2) at(std::size_t i, std::size_t j) const {return ex1.at(i)*ex2.at(i,j);}

  /// Size of the array
   std::size_t size() const {return ex2.size();}

  /// Number of columns
   std::size_t nbCols() const {return ex2.nbCols();}

  /// Number of rows
   std::size_t nbRows() const {return ex2.nbRows();}
...
template <typename V1, typename V2>
 inline VMMulT<V1,V2> operator* (EETYPE(V1) v1, MEETYPE(V2) v2)
{
...
public:
  ...
  /// Constructor
   MVMulVT (MEETYPE(V1) v1, EETYPE(V2) v2) :
    ExprT< MVMulVT<V1,V2>, ETPLVEC(V1,V2)>(this), ex1(v1), ex2(v2) {}

  /// Accessor to individual entry
   ETPLTYPE(V2) at(std::size_t i) const
  {
    ETPLTYPE(V2) res = ETPLTYPE(V2)();
    for (std::size_t k = 0; k < ex2.size(); ++k) {
...
template <typename V1, typename V2>
 inline MVMulVT<V1,V2> operator* (MEETYPE(V1) v1, EETYPE(V2) v2)
{
