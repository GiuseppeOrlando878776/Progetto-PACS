...
namespace Framework {
  ...
  void ReactingModelLibrary::SetRiGas(void) {
    Ri.clear();
    Ri.reserve(nSpecies);
    std::transform(mMasses.cbegin(),mMasses.cend(),std::back_inserter(Ri),[](double elem){return R_ungas/elem;});
  }
  ...
  double ReactingModelLibrary::ComputeEta(const double temp, const Vec& ys) {
    ...
    double phi;
    RealVec ys_over_mm;
    std::transform(ys.cbegin(),ys.cend(),mMasses.cbegin(),std::back_inserter(ys_over_mm),std::divides<double>());
  ...
  std::pair<double,double> ReactingModelLibrary::GetKeq(const double temp,const unsigned short iReac) {
    ...
    RealVec coeff_exp_reac = Stoich_Coeffs_Reactants_Exp.GetRow<RealVec>(iReac);
    RealVec conc_reac;
    ...
    if(Elementary_Reactions[iReac])
      ...
      kb = num;
      RealVec coeff_exp_prod = Stoich_Coeffs_Products_Exp.GetRow<RealVec>(iReac);
      RealVec conc_prod;
